...
namespace UPAEventsPayPal
{
    public class InstantPaymentNotification
    {
        ...
        public InstantPaymentNotification(HttpRequest request, string accountEmail, NameValueCollection form)
        {
        ...
        public bool ProcessIPNResults(HttpContext context, StreamWriter IPNWriter)
        {
            ...
            using (var con = new SqlConnection(System.Configuration.ConfigurationManager.ConnectionStrings["TeachersAssistant"].ConnectionString))
            {
            ...
            if (result)
            {
                ...
                string downloadInstructions = String.Format("http://www.martinlayooinc.com/Home/DownloadPage?username={0}&status=Download&product={1}", HttpUtility.UrlEncode(Username), HttpServerUtility.UrlTokenEncode(Encoding.GetEncoding(1252).GetBytes(prodBuffer.ToString())));
